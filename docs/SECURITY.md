...
**Honest status:**  
These are **prepared**. The AuthService/AuthController that *uses* them for login/register/forgot-password is **not implemented yet**.  
`TokenUtils` itself is currently unused — it waits for that Auth service.

That is normal in incremental learning: build the engine, then wire the dashboard.
...
- `POST /auth/register`  
- `POST /auth/login`  
- Refresh + forgot-password HTTP APIs  

When those land, Guest APIs (Module 4) become easy to call from Swagger with **Authorize**.

Next learning stop: [GUESTS.md](GUESTS.md)  
