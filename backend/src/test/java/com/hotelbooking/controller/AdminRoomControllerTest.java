...
class AdminRoomControllerTest {
    ...
    @Test
    void patchDescription_shouldReturn200() throws Exception {
        RoomDescriptionPatchRequest request = RoomDescriptionPatchRequest.builder()
                .description("Renovated sea view")
                .build();
        RoomResponse response = RoomResponse.builder().id(1L).description("Renovated sea view").build();
        when(roomService.patchDescription(eq(1L), any())).thenReturn(response);

        mockMvc.perform(patch("/admin/rooms/1/description")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.description").value("Renovated sea view"));
    }
